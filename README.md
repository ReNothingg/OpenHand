...
Далее появляется окно **«Переверните страницу»**: закрепите бумагу по прежнему
...
Кнопка **«Включить уведомления»** запрашивает разрешение на системные
...
В верхней навигации откройте **Плоттер**. Это один профиль и одно соединение для
документа и мастерской. В блоке «Положение пера» задаются ровно два значения:
**Перо поднято** и **Перо опущено**. Кнопка напротив каждого проверяет именно это
положение. Для шагового пера это абсолютные координаты Z/E; расстояние между ними
показано отдельно и не является третьей настройкой. Ввод числа не запускает
движение и не меняет второе значение. Скорость, механизм и короткие ручные шаги
находятся в «Дополнительно».
После потери опорного положения доступны «Сейчас поднято» и «Сейчас опущено»:
они привязывают текущую высоту к соответствующему введённому значению без движения,
перезаписи обоих чисел или изменения нуля листа.
При работе GRBL со шаговым Z приложение читает `$1` и включает удержание моторов
(`$1=255`), если оно отключено. Значение проверяется повторным чтением и сохраняется
в плате. Моторы остаются под током в паузах, чтобы пружинный держатель не терял
положение. Для Servo/EBB/Marlin этот параметр не меняется.
