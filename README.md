...
В разделе «Почерк» при включённом «Настоящем почерке» ползунок
«Автоматический разброс букв» меняет ширину, высоту и наклон повторений.
Для GFont также применяется небольшой плавный изгиб целой буквы. Случайный шум
к отдельным точкам траектории не добавляется; положения поднятого и опущенного
пера этот ползунок не меняет.

При загрузке и экспорте GFont удаляются повторно записанные пакеты точек,
распознанные по возврату к уже встречавшимся временным отметкам внутри штриха.
Обычные геометрические развороты и штрихи без временных отметок сохраняются.
Запись новых штрихов пера также отбрасывает повторные отсчёты времени.
...
Просмотр G-code и редактор шрифта используют тот же сеанс плоттера. При переходе
в них запись продолжается, а в шапке остаются её состояние, пауза и СТОП.
Окно смены бумаги появляется поверх текущего экрана. Возврат сохраняет документ,
начало листа и текущую настройку пера; повторное подключение для этого не требуется.
При открытии приложения сразу в просмотрщике порт сам не открывается.
...
В просмотрщике раздел **«Перо на предпросмотре»** позволяет выбрать ось Z/E,
сервопривод M3 S или M280 P0, либо включение M3/M4/M5. Для оси или сервопривода
задайте значения поднятого и опущенного пера из файла и нажмите
**«Применить к просмотру»**. Координаты осей здесь задаются в миллиметрах.
По умолчанию используются встроенные сведения OpenHand, а без них — приблизительное
определение. Эти параметры меняют только цвет и статистику траектории: сохранённый
G-code и профиль устройства остаются прежними. При открытии другого файла настройка
просмотра сбрасывается. Масштаб 100% вписывает рисунок в доступный холст; увеличение
сохраняет центр вида и позволяет прокручивать его до краёв.

Для GRBL короткие линейные движения отправляются с ограниченным заполнением
буфера платы, чтобы задержка каждого USB-ответа меньше мешала письму. Размер
буфера берётся из ответа платы; если она его не сообщает, используется обычная
последовательная отправка. Настройки, обнуление и ожидание смены листа идут
отдельно. При ошибке задания отправляется СТОП, управление блокируется, а последняя
подтверждённая точка сохраняется. Нажатие пользователем СТОП удаляет продолжение.
...
В оболочке версии 7 перезапуск страницы или сбой её процесса сначала запускает
нативный СТОП. На macOS закрытие порта ждёт передачи выходного буфера; если она
не подтверждена за две секунды, порт остаётся открытым и показывается ошибка.
Восстановленный интерфейс читает блокировку из оболочки и не возобновляет очередь.
При ошибке доставки СТОП причина остаётся видимой, а новое подключение требует
явного разрешения управления. Без активного соединения восстановление интерфейса
не открывает сохранённый USB-порт. Старые ответы оболочки отделены от запросов
новой страницы, включая ответы сохранения файлов.

В оболочке версии 8 данные и команды привязаны к конкретному подключению. Поздний
ответ, отключение или закрытие старого порта не затрагивают новое соединение.
СТОП отменяет также ожидающие записи интерфейса: снятие блокировки не возвращает
их в очередь. Запрос текущего состояния остаётся доступным, а для последующего
управления создаётся новое соединение. Переподключение ждёт завершения закрытия
предыдущего порта.
...
Ручные стрелки GRBL после установки начала листа проверяют весь шаг по свежим
рабочим координатам и заданным размерам области. Выходящий за неё шаг не отправляется
и не сокращается незаметно. Из положения за границей разрешено движение обратно
в область. Это проверка настроенных координат, а не датчик физического края.
До установки начала стрелки позволяют подвести каретку к листу. Кнопка
**«Переставить начало листа»** снимает только эту привязку в приложении, без движения
и без изменения настройки пера. Для других прошивок такая проверка текущих
координат GRBL не применяется.
