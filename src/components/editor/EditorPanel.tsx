...
const MARKDOWN_TOOLS = [
  ...
  {
    label: "⇤",
    title: "Блок по левому краю",
    kind: "block",
    before: ":::left",
    after: ":::",
    placeholder: "Текст по левому краю",
    className: "is-alignment is-alignment-start",
  },
  {
    label: "↔",
    title: "Блок по центру",
    kind: "block",
    before: ":::center",
    after: ":::",
    placeholder: "Текст по центру",
    className: "is-alignment",
  },
  {
    label: "⇥",
    title: "Блок по правому краю",
    kind: "block",
    before: ":::right",
    after: ":::",
    placeholder: "Текст по правому краю",
    className: "is-alignment",
  },
];
...
const TEX_TOOLS = [
  ...
  {
    label: "≡",
    title: "Формула отдельной строкой",
    before: "\\[\n",
    after: "\n\\]",
    placeholder: "E = mc^2",
  },
...
const MARKDOWN_HELP_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      [
        "Размещение",
        ":::place id=note page=2 x=40 y=70 width=320 align=center\nТекст\n:::",
      ],
...
function cleanSvg(source) {
  ...
  if (!clean.includes("<svg"))
    throw new Error("В SVG не осталось допустимого содержимого.");
  return clean;
}
...
export default function EditorPanel({
  ...
  const updateSelection = ({
    ...
    if (kind === "line") {
      const lineStart =
        activeSource.lastIndexOf("\n", Math.max(0, start - 1)) + 1;
      const nextLineBreak = activeSource.indexOf("\n", end);
      const lineEnd =
        nextLineBreak === -1 ? activeSource.length : nextLineBreak;
      ...
      replacement = block
        ...
        .join("\n");
      setActiveSource(
        `${activeSource.slice(0, lineStart)}${replacement}${activeSource.slice(lineEnd)}`,
      );
      ...
      const leadingBreak =
        start > 0 && activeSource[start - 1] !== "\n" ? "\n\n" : "";
      const trailingBreak =
        end < activeSource.length && activeSource[end] !== "\n" ? "\n\n" : "";
      replacement = `${leadingBreak}${before}\n${content}\n${after}${trailingBreak}`;
      setActiveSource(
        `${activeSource.slice(0, start)}${replacement}${activeSource.slice(end)}`,
      );
  ...
  const handleEditorKeyDown = (
    ...
    if (event.key === "Tab") {
      ...
      const end = target.selectionEnd;
      setActiveSource(
        `${activeSource.slice(0, start)}  ${activeSource.slice(end)}`,
      );
      requestAnimationFrame(() =>
  ...
  const importSvg = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    try {
      if (file.size > 5 * 1024 * 1024)
        throw new Error("SVG больше 5 МБ — сначала упростите его.");
      ...
      <div className="panel-title editor-title">
        ...
        >
          ...
          <button
            type="button"
            role="tab"
            aria-selected={sourceMode === "tex"}
            className={sourceMode === "tex" ? "active" : ""}
            onClick={() => setSourceMode("tex")}
          >
            TeX
          </button>
        </div>
        ...
        createPortal(
          ...
          >
            <section
              ...
              role="dialog"
              aria-modal="true"
              aria-label="Большой редактор текста"
            >
              <header>
                ...
                <div className="expanded-editor-header-actions">
                  <div
                    className="source-tabs"
                    role="tablist"
                    aria-label="Формат большого редактора"
                  >
                    <button
                      type="button"
                      role="tab"
                  ...
                  <button
                    className="expanded-editor-close"
                    type="button"
                    onClick={() => setExpanded(false)}
                    aria-label="Закрыть большой редактор"
                    title="Закрыть · Esc"
                  >
                    <Icon name="close" />
                  </button>
              ...
              <div className="expanded-editor-toolbar">
                ...
                >
                  {(sourceMode === "tex" ? TEX_TOOLS : MARKDOWN_TOOLS).map(
                    (tool) => (
                      <button
                        key={tool.title}
                        type="button"
              ...
              <div className="expanded-editor-body">
                <textarea
                  ...
                  onChange={(event) => setActiveSource(event.target.value)}
                  onKeyDown={handleEditorKeyDown}
                  aria-label="Большое поле редактирования"
                />
                {helpOpen && (
                  <aside id="editor-help" aria-label="Справка по синтаксису">
                    <div className="editor-help-heading">
                      <div>
                        <strong>Синтаксис</strong>
                        <small>Короткая шпаргалка</small>
                      </div>
                      <button
                        type="button"
              ...
              <footer>
                <div>
                  ...
                  <span>{characterCount.toLocaleString("ru-RU")} знаков</span>
                </div>
                <span>
                  <kbd>Esc</kbd> закрыть
                </span>
