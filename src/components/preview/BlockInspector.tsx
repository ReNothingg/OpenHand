  ...
  if (!selected)
    return (
      <div className="block-inspector empty">
        ...
        <code>
          :::place id=name page=2 x=40 y=70 width=380 height=120 rotate=-2
          align=center nowrap
        </code>
      </div>
    );
  ...
  return (
    <div className="block-inspector" aria-label="Свойства выбранного блока">
      <div className="block-identity">
        <span className={`block-kind ${block.kind}`}>
          {block.kind === "formula" ? "ƒ" : block.kind === "svg" ? "◇" : "T"}
        </span>
        <div>
          <strong>{block.label}</strong>
          <small>Лист {pageIndex + 1}</small>
        </div>
      </div>
      <label>
        <span>Лист</span>
        <input
          type="number"
          min="1"
          value={(layout.pageIndex ?? pageIndex) + 1}
          onChange={(event) =>
            update({ pageIndex: Math.max(0, Number(event.target.value) - 1) })
          }
        />
      </label>
      <label>
      ...
      <div className="alignment-control" role="group" aria-label="Выравнивание">
        {["left", "center", "right"].map((align) => (
          <button
            type="button"
            className={layout.align === align ? "active" : ""}
      ...
      <label className="nowrap-control">
        <input
          type="checkbox"
          checked={layout.noWrap}
          onChange={(event) => update({ noWrap: event.target.checked })}
        />
        <span>Не переносить</span>
      </label>
      <button
        className="text-button"
        type="button"
        onClick={() => onReset(originPage, block.id)}
      >
        Сбросить
      </button>
    </div>
