...
function findPageAtPoint(clientX: number, clientY: number): HTMLElement | undefined {
  const pages = [...document.querySelectorAll<HTMLElement>('.manual-page-content[data-page-index]')]
  ...
  return document.elementsFromPoint(clientX, clientY)
    .find((element): element is HTMLElement => element instanceof HTMLElement && element.matches('.manual-page-content[data-page-index]'))
}
...
export default function ManualPageContent({
  ...
  useLayoutEffect(() => {
    ...
    const measurements = {}
    hostRef.current.querySelectorAll<HTMLElement>('[data-manual-block]').forEach((element) => {
  ...
  const beginInteraction = (event, block, mode) => {
    ...
    const move = (moveEvent) => {
      ...
      if (mode === 'move') {
        const patch = {
          x: clamp(start.blockX + dx, -start.width + 20, contentStyle.width - 20),
          y: clamp(start.blockY + dy, -start.height + 20, contentStyle.height - 20),
        }
        ...
        if (viewport) {
          ...
          else if (moveEvent.clientY > bounds.bottom - edge) viewport.scrollTop += 18
          if (moveEvent.clientX < bounds.left + edge) viewport.scrollLeft -= 18
          else if (moveEvent.clientX > bounds.right - edge) viewport.scrollLeft += 18
        }
        document.querySelectorAll<HTMLElement>('.manual-page-content.is-drop-target').forEach((element) => element.classList.remove('is-drop-target'))
        const target = findPageAtPoint(moveEvent.clientX, moveEvent.clientY)
        if (target && Number(target.dataset.pageIndex) !== pageIndex) target.classList.add('is-drop-target')
      } else {
        const patch = {
        ...
        if (target) {
          ...
          if (Number.isFinite(targetPage) && targetPage !== pageIndex) {
            ...
            const patch = {
              ...
              x: clamp((endEvent.clientX - bounds.left) / scale - start.grabX, -start.width + 20, contentStyle.width - 20),
              y: clamp((endEvent.clientY - bounds.top) / scale - start.grabY, -start.height + 20, contentStyle.height - 20),
            }
            latestLayout = { ...latestLayout, ...patch }
            onUpdate(originPage, block.id, patch)
            onSelect({ pageIndex: targetPage, block: { ...block, layout: { ...current, pageIndex: targetPage } } })
          }
        }
      }
  ...
  return (
    ...
    >
      {blocks.map((block) => {
        ...
        const active = selected?.pageIndex === pageIndex && selected?.block?.id === block.id
        ...
        return (
          ...
          >
            <div className="manual-block-body" dangerouslySetInnerHTML={{ __html: block.html }} />
            {editing && <span className="manual-block-kind" aria-hidden="true">{block.kind === 'formula' ? 'ƒ' : block.kind === 'svg' ? '◇' : 'T'}</span>}
            {editing && <button className="manual-resize-handle" type="button" aria-label={`Изменить размер: ${block.label}`} onPointerDown={(event) => beginInteraction(event, block, 'resize')} />}
          </div>
        )
      })}
    </div>
