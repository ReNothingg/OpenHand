...
export default function FontCanvas({ character, strokes, onChange }) {
  ...
  const begin = (event) => {
    ...
    onChange([...baseStrokesRef.current, drawingRef.current], { transient: true })
  }
  ...
  const end = () => {
    ...
    if (stroke.length > 1) onChange([...baseStrokesRef.current, stroke], { previous: baseStrokesRef.current })
