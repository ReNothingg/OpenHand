...
function strokePath(stroke: FontStroke, offsetX: number, minX: number, scale: number, baseline: number) {
  return stroke.map((point, index) => (
    `${index === 0 ? 'M' : 'L'} ${offsetX + (point.x - minX) * scale} ${baseline + point.y * scale}`
  )).join(' ')
}

export default function FontPreview({ text, glyphs, size = 32 }: { text: string; glyphs: Record<string, FontStroke[]>; size?: number }) {
