...
export default function PenTools({ settings, onChange, tool, onToolChange }: {
  ...
  return (
    <div className="font-pen-tools" role="group" aria-label="Инструменты пера">
      <div className="font-tool-group" role="group" aria-label="Режим рисования">
        <button type="button" aria-pressed={tool === "pen"} onClick={() => onToolChange("pen")}>
          Перо
        </button>
        <button type="button" aria-pressed={tool === "eraser"} onClick={() => onToolChange("eraser")} title="Удаляет штрих целиком">
          Штрих
        </button>
        <button type="button" aria-pressed={tool === 'trim'} onClick={() => onToolChange('trim')} title="Стирает только часть линии">
          Фрагмент
        </button>
      </div>
