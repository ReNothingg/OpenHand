    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      for (let y = 1; y < height - 1; y += 1) {
        for (let x = 1; x < width - 1; x += 1) {
          ...
          const transitions = ring
            .slice(0, 8)
            .reduce(
              (sum, value, ringIndex) =>
                sum + (value === 0 && ring[ringIndex + 1] === 1 ? 1 : 0),
              0,
            );
...
function traceSkeleton(binary, width, height) {
  ...
  const neighborsFor = (point) =>
    offsets
      .map(([dx, dy]) => ({
        x: point.x + dx,
        y: point.y + dy,
        index: (point.y + dy) * width + point.x + dx,
      }))
  ...
  const walk = (start, first) => {
    ...
    while (current) {
      stroke.push(current);
      const candidates = neighborsFor(current).filter(
        (candidate) =>
          candidate.index !== previous.index &&
          !used.has(edgeKey(current.index, candidate.index)),
      );
...
function vectorizeImageData(
  ...
  removeSmallComponents(
    ...
    width,
    height,
    Math.max(4, Math.round(width * height * 0.00035)),
  );
...
function drawScaled(bitmap: ImageBitmap | HTMLImageElement, maximum = 1800) {
  const sourceWidth =
...
function warpSheet(source, corners) {
  ...
  for (let y = 0; y < canvas.height; y += 1) {
    ...
    for (let x = 0; x < canvas.width; x += 1) {
      ...
      const sourceX = clamp(
        Math.round(leftX + (rightX - leftX) * u),
        0,
        source.width - 1,
      );
      const sourceY = clamp(
        Math.round(leftY + (rightY - leftY) * u),
        0,
        source.height - 1,
      );
...
export function createPhotoTemplateSvg(characters) {
  ...
  const markers = [
    ...
    .map(
      ([x, y]) =>
        `<rect x="${x - 22}" y="${y - 22}" width="44" height="44" fill="#000"/>`,
    )
    .join("");
...
export async function vectorizeSinglePhoto(file: File) {
  const bitmap = await bitmapFromFile(file);
  const canvas = drawScaled(bitmap, 900);
  const context = canvas.getContext("2d", { willReadFrequently: true });
  return vectorizeImageData(
...
export async function vectorizePhotoSheet(
  ...
  const sheet = corners
    ? warpSheet(source, corners)
    : tightlyCropToSheet(source);
  const rows = Math.ceil(characters.length / SHEET.columns);
  const gridWidth = SHEET.width - SHEET.insetX * 2;
  const gridHeight = SHEET.height - SHEET.insetTop - SHEET.insetBottom;
  const cellWidth = gridWidth / SHEET.columns;
  ...
  const glyphs: Record<string, Array<Array<{ x: number; y: number }>>> = {};
  for (let index = 0; index < characters.length; index += 1) {
    ...
    if (index % 4 === 0 || index === characters.length - 1) {
      onProgress({
        phase: "glyphs",
        progress: 0.1 + ((index + 1) / characters.length) * 0.9,
      });
