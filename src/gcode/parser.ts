...
function boundsFor(segments) {
  if (!segments.length) return { minX: 0, minY: 0, maxX: 210, maxY: 297, width: 210, height: 297 }
  let minX = Infinity
  let minY = Infinity
  let maxX = -Infinity
  let maxY = -Infinity
...
export function parseGCode(source = '') {
  ...
  lines.forEach((rawLine, index) => {
    ...
    const hasPlanarMove = coordinates.x !== undefined || coordinates.y !== undefined
    if (hasPlanarMove && motion !== null) {
      const next = {
        x: coordinates.x === undefined ? position.x : (absolute ? coordinates.x : position.x + coordinates.x),
        y: coordinates.y === undefined ? position.y : (absolute ? coordinates.y : position.y + coordinates.y),
        z: coordinates.z === undefined ? position.z : (absolute ? coordinates.z : position.z + coordinates.z),
      }
    ...
    commandCount += words.filter((word) => ['G', 'M'].includes(word.letter)).length
  })
  ...
  return {
    ...
    commandCount,
    ignoredLines,
    drawDistance: drawing.reduce((sum, segment) => sum + segmentLength(segment), 0),
    travelDistance: travel.reduce((sum, segment) => sum + segmentLength(segment), 0),
  }
}
