...
export async function loadStoredGFonts(): Promise<StoredGFont[]> {
  ...
  try {
    ...
    const records = await requestResult<StoredGFont[]>(transaction.objectStore(STORE_NAME).getAll())
...
export async function saveStoredGFont(name: string, buffer: ArrayBuffer): Promise<StoredGFont> {
