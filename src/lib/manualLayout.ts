...
export function htmlToManualBlocks(html, pageIndex, includePlotterText = true) {
  ...
  host.innerHTML = html;
  const nodes = Array.from(host.children) as Array<HTMLElement | SVGElement>;
  ...
  return nodes.map((node, index) => {
    ...
    const kind =
      ...
      node.matches("svg, figure.imported-svg, .tex-tikz")
        ? "svg"
        : node.querySelector(".katex, math") ||
            node.matches(".katex-display, math")
          ? "formula"
          : "text";
    ...
    const defaultLayout = layoutId
      ? {
          ...
          align: ["left", "center", "right"].includes(node.dataset.layoutAlign)
            ? node.dataset.layoutAlign
            : "left",
    ...
    return {
      id: layoutId
        ? `md-${layoutId}`
        : `${pageIndex}-${index}-${hash(blockHtml)}`,
...
export function arrangeManualPages(sourcePages, layouts) {
  ...
  sourcePages.forEach((blocks, originPage) => {
    blocks.forEach((block) => {
      const layout =
        layouts[originPage]?.[block.id] || block.defaultLayout || null;
...
export function normalizeBlockLayout(
  ...
  return {
    ...
    align: ["left", "center", "right"].includes(layout.align)
      ? layout.align
      : "left",
    noWrap: Boolean(layout.noWrap),
    pageIndex: Number.isFinite(Number(layout.pageIndex))
      ? Math.max(0, Math.round(Number(layout.pageIndex)))
      : undefined,
  };
}
...
export function updatePlacementDirective(markdown, layoutId, incomingLayout) {
  ...
  return String(markdown)
    .split("\n")
    .map((sourceLine) =>
      sourceLine.startsWith(":::place") &&
      directiveId(sourceLine.slice(":::place".length)) === layoutId
        ? line
        : sourceLine,
    )
    .join("\n");
