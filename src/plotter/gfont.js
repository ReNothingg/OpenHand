...
export class GFont {
  constructor(arrayBuffer, name = 'Шрифт .gfont') {
...
export async function loadGFont(source, name) {
  const buffer = source instanceof ArrayBuffer ? source : await source.arrayBuffer()
  return new GFont(buffer, name || source.name)
}

function transformGlyph(glyph, codePoint, transform = {}) {
