...
function splitGlyphStrokes(glyph, cursorX, baseline, scale, handwriting = null): PlotStroke[] {
...
export async function layoutText(text: string, font: any, page: any, config: any) {
  ...
  const widthForLine = (line) => {
    ...
    return line
      ...
      .reduce((width, token) => {
        ...
        return width + Array.from(String(token)).reduce((total, char) => {
...
export async function layoutBlocks(blocks: any[], font: any, page: any, config: any) {
  const strokes: PlotStroke[] = []
  const missing = new Set<string>()
  const clippedItems = []
  let clipped = false
