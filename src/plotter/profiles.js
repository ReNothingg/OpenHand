import { DEFAULT_PLOTTER_CONFIG } from './job.js'
...
export function normalizePlotterConfig(incoming = {}) {
...
export function createPlotterProfile(name, config = {}, options = {}) {
...
export function normalizePlotterProfile(incoming, options = {}) {
...
export function createPlotterProfileStore(rawStore, legacyConfig, options = {}) {
  const now = Number.isFinite(options.now) ? options.now : Date.now()
  if (
