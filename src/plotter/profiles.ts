...
export function normalizePlotterConfig(incoming: Record<string, any> = {}) {
  ...
  const penModes = profile === 'ebb'
    ? ['servo']
    : profile === 'marlin'
      ? ['servo', 'stepper', 'estepper']
      : ['servo', 'stepper', 'laser']
  const servoMax = profile === 'marlin' ? 180 : 32767
  const fallbackPenUp = profile === 'marlin' ? 50 : DEFAULT_PLOTTER_CONFIG.penUp
  const fallbackPenDown = profile === 'marlin' ? 0 : DEFAULT_PLOTTER_CONFIG.penDown
  ...
  return {
    ...
    feedRate: clamp(incoming.feedRate, 1, 10000, DEFAULT_PLOTTER_CONFIG.feedRate),
    jogSpeed: clamp(incoming.jogSpeed, 1, 10000, DEFAULT_PLOTTER_CONFIG.jogSpeed),
    jogDistance: clamp(incoming.jogDistance, 0.1, 50, DEFAULT_PLOTTER_CONFIG.jogDistance),
    ...
    laserPower: clamp(incoming.laserPower, 0, 1000, DEFAULT_PLOTTER_CONFIG.laserPower),
    mmToSteps: clamp(incoming.mmToSteps, 1, 1000, DEFAULT_PLOTTER_CONFIG.mmToSteps),
    penDelay: clamp(incoming.penDelay, 0, 10, DEFAULT_PLOTTER_CONFIG.penDelay),
    letterSpacing: clamp(incoming.letterSpacing, 0, 20, DEFAULT_PLOTTER_CONFIG.letterSpacing),
    optimizePath: boolean(incoming.optimizePath, DEFAULT_PLOTTER_CONFIG.optimizePath),
...
export function createPlotterProfile(name: unknown, config: Record<string, any> = {}, options: Record<string, any> = {}) {
  ...
  return {
    ...
    name: String(name || PROFILE_NAMES[normalizedConfig.profile] || 'Плоттер').trim().slice(0, 64) || 'Плоттер',
    config: normalizedConfig,
    calibratedAt: Number.isFinite(options.calibratedAt) ? options.calibratedAt : null,
...
export function normalizePlotterProfile(incoming: Record<string, any>, options: Record<string, any> = {}) {
...
export function createPlotterProfileStore(rawStore: any, legacyConfig: any, options: Record<string, any> = {}) {
  const now = Number.isFinite(options.now) ? options.now : Date.now()
  if (
    ...
    const profiles = rawStore.profiles.map((profile) => normalizePlotterProfile(profile, {
      id: typeof profile?.id === 'string' ? profile.id : undefined,
      now,
    }))
    const activeProfileId = profiles.some((profile) => profile.id === rawStore.activeProfileId)
      ? rawStore.activeProfileId
      : profiles[0].id
    return { version: PLOTTER_PROFILES_VERSION, activeProfileId, profiles }
  }

  const profile = createPlotterProfile('Текущий плоттер', legacyConfig || {}, { now })
  return {
...
export function loadPlotterProfileStore(storage = localStorage) {
  ...
  try {
    legacyConfig = JSON.parse(storage.getItem(LEGACY_PLOTTER_SETTINGS_KEY) || 'null')
  } catch {
    legacyConfig = null
  }
