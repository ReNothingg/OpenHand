...
function formulaSource(formula: Element) {
  const annotation = formula
    .querySelector('annotation[encoding="application/x-tex"], annotation')
    ?.textContent?.trim();
  ...
  container.querySelectorAll(".katex").forEach((formula) => {
    ...
    formula.replaceWith(
      document.createTextNode(
        normalized
          ? `${PLOTTER_FORMULA_START}${normalized}${PLOTTER_FORMULA_END}`
          : "",
      ),
    );
  });
...
function tableDrawing(table: HTMLTableElement) {
  ...
  const columnWidths = Array.from({ length: columnCount }, (_, columnIndex) => {
    const contentWidth = Math.max(
      ...
      ...tableValues.map((values) =>
        estimateTableTextWidth(values[columnIndex] || ""),
      ),
    );
  ...
  for (let columnIndex = 0; columnIndex <= columnCount; columnIndex += 1) {
    ...
    strokes.push(
      handwrittenTableLine(x, 0, x, height, columnIndex + rows.length + 1),
    );
  }
...
function splitDashedStroke(points, pattern) {
  if (points.length < 2 || !pattern.length)
...
function svgTextDrawing(text, bounds) {
  ...
  if (
    ...
    return null;
  const x = text.x?.baseVal?.numberOfItems
    ? text.x.baseVal.getItem(0).value
    : Number(text.getAttribute("x")) || 0;
  const y = text.y?.baseVal?.numberOfItems
    ? text.y.baseVal.getItem(0).value
    : Number(text.getAttribute("y")) || 0;
...
function svgDrawing(svg) {
  ...
  host.style.cssText =
    "position:fixed;left:-100000px;top:0;width:800px;height:auto;opacity:0;pointer-events:none";
  ...
  try {
    ...
    clone
      ...
      .forEach((geometry) => {
        ...
        const isBackground =
          geometry.tagName.toLowerCase() === "rect" &&
          !strokeVisible &&
        ...
        for (let index = 0; index <= steps; index += 1) {
          ...
          const screen = new DOMPoint(source.x, source.y).matrixTransform(
            matrix,
          );
        ...
        const dashPattern =
          strokeVisible && style.strokeDasharray !== "none"
            ? style.strokeDasharray
                .split(/[,\s]+/)
                .map(Number.parseFloat)
                .filter((value) => value > 0)
                .map((value) => value * matrixScale)
            : [];
        geometryStrokes.forEach((item) =>
          strokes.push(...splitDashedStroke(item, dashPattern)),
        );
        ...
        const arrowSize = Math.max(
          ...
          Math.min(
            18,
            (Number.parseFloat(style.strokeWidth) || 1) * matrixScale * 5,
          ),
        );
        if (
          strokeVisible &&
          style.markerStart !== "none" &&
          firstStroke?.length > 1
        ) {
    ...
    return strokes.length || texts.length
      ? { width: bounds.width, height: bounds.height, strokes, texts }
      : null;
  } finally {
    host.remove();
  }
...
function listMarker(item: HTMLLIElement) {
  ...
  const index = Array.from(list.children)
...
export function htmlToPlotterText(html) {
  ...
  const appendBeforeTrailingBreaks = (value) => {
    ...
    if (trailing)
      output = `${output.slice(0, -trailing.length)}${value}${trailing}`;
    else output += value;
  };
