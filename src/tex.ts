...
function repairMathSource(source) {
  return source
    .replace(/\\\[([\s\S]*?)\\\]/g, (_match, formula) => `\\[${repairIncompleteFormula(formula, source)}\\]`)
    .replace(/\\\(([\s\S]*?)\\\)/g, (_match, formula) => `\\(${repairIncompleteFormula(formula, source)}\\)`)
    .replace(/\$([^$\n]+)\$/g, (_match, formula) => `$${repairIncompleteFormula(formula, source)}$`)
}
...
function prepareSource(source) {
  ...
  const circuits = []
  let prepared = source
  const documentBody = prepared.match(/\\begin\{document\}([\s\S]*?)\\end\{document\}/)
  if (documentBody) {
  ...
  prepared = prepared.replace(
    ...
    (_match, options = '', contents) => {
      const language = options.match(/language\s*=\s*([^,\]]+)/)?.[1]?.trim() || 'text'
      const caption = options.match(/caption\s*=\s*\{([^}]*)\}/)?.[1]?.trim() || ''
      const index = codeBlocks.push({ language, caption, contents: contents.replace(/^\s*\n|\n\s*$/g, '') }) - 1
  ...
  prepared = prepared.replace(
    ...
    (_match, _options, number, contents) => {
      const index = boxes.push({ kind: 'taskbox', title: `Задача ${number}`, contents }) - 1
      return `\\par ${BOX_TOKEN_PREFIX}${index} \\par`
    },
  )
  ;[
    ...
    prepared = prepared.replace(pattern, (_match, subtitle = '', contents) => {
      const index = boxes.push({ kind: environment, title: subtitle ? `${title}: ${subtitle}` : title, contents }) - 1
      return `\\par ${BOX_TOKEN_PREFIX}${index} \\par`
    })
  })
  prepared = prepared.replace(
  ...
  prepared = prepared.replace(
    ...
    (_match, columns, contents) => {
      const index = tables.push({ columns, contents: contents.replace(/\\endfirsthead|\\endhead/g, '') }) - 1
      return `\\par ${TABLE_TOKEN_PREFIX}${index} \\par`
    },
  )
  prepared = prepared.replace(
  ...
  prepared = prepared.replace(
    ...
    (_match, count, heading = '', contents) => {
      const index = multicols.push({ count: Number(count), heading, contents }) - 1
  ...
  prepared = prepared.replace(/\\(?:tableofcontents|listoffigures|listoftables)\b/g, '')
  prepared = prepared.replace(
    /\\tikzset\{[\s\S]*?(?=\\begin\{tikzpicture\}|\\par\s+HANDWRITERTIKZTOKEN)/g,
    '',
  )
  prepared = prepared.replace(/\\begin\{(?:titlepage|figure|table)\}(?:\[[^\]]*\])?/g, '')
  prepared = prepared.replace(/\\end\{(?:titlepage|figure|table)\}/g, '')
  prepared = prepared.replace(/\\begin\{enumerate\}\[[^\]]*\]/g, '\\begin{enumerate}')
  prepared = prepared.replace(/\\begin\{thebibliography\}\{[^}]*\}/g, '\\section*{Литература}')
  ...
  prepared = prepared.replace(/\\caption\{([^{}]*)\}/g, '\\par \\textit{$1} \\par')
  prepared = prepared.replace(/\\label\{[^}]*\}/g, '')
  prepared = prepared.replace(/\\(?:eqref|ref|pageref|cite)\{([^}]*)\}/g, '[$1]')
  prepared = prepared.replace(/\\renewcommand\{[^}]+\}(?:\[[^\]]*\])?\{[^}]*\}/g, '')
  prepared = prepared.replace(
  ...
  prepared = prepared.replace(/\\rule\{[^}]*\}\{[^}]*\}/g, `\\par ${RULE_TOKEN} \\par`)
  prepared = prepared.replace(
    /\\(?:newpage|clearpage|pagebreak)(?:\[[^\]]*\])?/g,
    `\\par ${PAGE_BREAK_TOKEN} \\par`,
  )
  return { prepared, tables, multicols, tikzPictures, codeBlocks, boxes, circuits }
}
...
function normalizeCompatCommands(source) {
  return source
    .replace(/\\SI\{([^{}]*)\}\{([^{}]*)\}/g, (_match, value, unit) => `${value} ${unitText(unit)}`)
...
function renderFragment(source) {
  ...
  const result = parse(repairMathSource(normalizeCompatCommands(source)), { generator })
...
function makeTable({ columns, contents }) {
  ...
  const alignments = [...columns].filter((character) => /[lcr]/.test(character))
...
function makeCircuit() {
  ...
  const svg = svgNode('svg', { viewBox: '0 0 560 300', role: 'img', 'aria-label': 'Электрическая схема' })
  const wire = (x1, y1, x2, y2) => svg.append(svgNode('line', { x1, y1, x2, y2, stroke: 'currentColor', 'stroke-width': 2.2, 'stroke-linecap': 'round' }))
  const label = (x, y, value) => {
    const node = svgNode('text', { x, y, fill: 'currentColor', 'font-size': 21, 'text-anchor': 'middle' })
    node.textContent = value
    svg.append(node)
  }
  wire(70, 55, 490, 55); wire(490, 55, 490, 245); wire(490, 245, 70, 245); wire(70, 245, 70, 55)
  wire(250, 55, 250, 245); wire(250, 150, 490, 150)
  ;[[70, 150, 'ℰ'], [175, 55, 'K'], [350, 55, 'R₁'], [490, 105, 'R₂'], [250, 115, 'A'], [370, 150, 'L'], [490, 205, 'V']].forEach(([x, y, value]) => label(x, y, value))
  wrapper.append(svg)
  return wrapper
}
...
function makeMulticols({ count, heading, contents }) {
  ...
  grid.style.setProperty('--tex-columns', String(Math.max(1, Math.min(4, count || 2))))
  repairMathSource(contents).split(/\\columnbreak/g).forEach((columnSource) => {
...
function parseDrawCommands(contents) {
  return contents.split(';').flatMap((source) => {
    ...
    const line = source.match(/\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)\s*--\s*\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)/)
    const arc = source.match(/\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)\s*arc\s*\((-?\d*\.?\d+):(-?\d*\.?\d+):(-?\d*\.?\d+)\)/)
    const rectangle = source.match(/\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)\s*rectangle\s*\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)/)
    const circle = source.match(/\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)\s*circle\s*\((-?\d*\.?\d+)\)/)
    const labelMatch = source.match(/node\[([^\]]*)\]\s*\{([\s\S]*)\}\s*$/)
    const label = labelMatch ? { position: labelMatch[1], text: plainTexLabel(labelMatch[2]) } : null
    if (line) {
      return [{ type: 'line', options, start: [Number(line[1]), Number(line[2])], end: [Number(line[3]), Number(line[4])], label }]
    }
    if (arc) {
      return [{ type: 'arc', options, start: [Number(arc[1]), Number(arc[2])], angles: [Number(arc[3]), Number(arc[4])], radius: Number(arc[5]), label }]
    }
    if (rectangle) {
      return [{ type: 'rectangle', options, start: [Number(rectangle[1]), Number(rectangle[2])], end: [Number(rectangle[3]), Number(rectangle[4])], label }]
    }
    if (circle) {
      return [{ type: 'circle', options, start: [Number(circle[1]), Number(circle[2])], radius: Number(circle[3]), label }]
    }
    return []
  })
}
...
function makeFlowchart(contents) {
  const labels = [...contents.matchAll(/\\node(?:\s*\([^)]*\))?\s*(?:\[[^\]]*\])?[^{}]*\{([^{}]*)\}/g)]
  ...
  const svg = svgNode('svg', { viewBox: `0 0 ${width} ${height}`, role: 'img', 'aria-label': 'Блок-схема TikZ' })
  labels.forEach((value, index) => {
    const y = 18 + index * rowHeight
    if (index) {
      svg.append(svgNode('line', { x1: width / 2, y1: y - 20, x2: width / 2, y2: y - 4, stroke: 'currentColor', 'stroke-width': 2 }))
      svg.append(svgNode('path', { d: `M ${width / 2 - 5} ${y - 10} L ${width / 2} ${y - 3} L ${width / 2 + 5} ${y - 10}`, fill: 'none', stroke: 'currentColor', 'stroke-width': 2 }))
    }
    svg.append(svgNode('rect', { x: 105, y, width: 310, height: 42, rx: index === 0 || index === labels.length - 1 ? 18 : 6, fill: 'none', stroke: 'currentColor', 'stroke-width': 2 }))
    const text = svgNode('text', { x: width / 2, y: y + 27, fill: 'currentColor', 'font-size': 20, 'text-anchor': 'middle' })
...
function resolveTikzCoordinates(contents) {
  ...
  for (const match of contents.matchAll(/\\coordinate\s*\(([^)]+)\)\s*at\s*\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)/g)) {
    coordinates.set(match[1], [Number(match[2]), Number(match[3])])
  }
  for (const match of contents.matchAll(/\\coordinate\s*\(([^)]+)\)\s*at\s*\(\$\(([^)]+)\)\+\(([^)]+)\)\$\)/g)) {
    const left = coordinates.get(match[2])
    const right = coordinates.get(match[3])
    if (left && right) coordinates.set(match[1], [left[0] + right[0], left[1] + right[1]])
  }
...
function makeForceDiagram(contents) {
  ...
  commands.forEach((command) => {
    ...
    if (command.type === 'circle') {
      points.push([command.start[0] - command.radius, command.start[1] - command.radius])
      points.push([command.start[0] + command.radius, command.start[1] + command.radius])
    }
    if (command.type === 'arc') {
      const [startAngle, endAngle] = command.angles.map((angle) => angle * Math.PI / 180)
      const center = [command.start[0] - command.radius * Math.cos(startAngle), command.start[1] - command.radius * Math.sin(startAngle)]
      points.push([center[0] + command.radius * Math.cos(endAngle), center[1] + command.radius * Math.sin(endAngle)])
    }
  })
  ...
  const map = ([x, y]: [number, number]): [number, number] => [padding + (x - minX) * scale, height - padding - (y - minY) * scale]
  const svg = svgNode('svg', { viewBox: `0 0 ${width} ${height}`, role: 'img', 'aria-label': 'Схема TikZ' })
  const defs = svgNode('defs')
  const marker = svgNode('marker', { id: 'tex-arrow', viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 6, markerHeight: 6, orient: 'auto-start-reverse' })
  marker.append(svgNode('path', { d: 'M 0 0 L 10 5 L 0 10 z', fill: 'context-stroke' }))
  defs.append(marker)
  svg.append(defs)

  commands.forEach((command) => {
    const color = tikzColor(command.options)
    const widthValue = /very\s+thick/.test(command.options) ? 3.2 : /thick/.test(command.options) ? 2.2 : 1.5
    let labelPoint
    if (command.type === 'line') {
      const [x1, y1] = map(command.start)
      const [x2, y2] = map(command.end)
      const line = svgNode('line', { x1, y1, x2, y2, stroke: color, 'stroke-width': widthValue, 'stroke-linecap': 'round' })
      if (/dashed/.test(command.options)) line.setAttribute('stroke-dasharray', '8 6')
      if (/->/.test(command.options)) line.setAttribute('marker-end', 'url(#tex-arrow)')
      svg.append(line)
      labelPoint = (/midway/.test(command.label?.position || '') ? [(x1 + x2) / 2, (y1 + y2) / 2] : [x2, y2]) as [number, number]
    } else if (command.type === 'arc') {
      const startAngle = command.angles[0] * Math.PI / 180
      const endAngle = command.angles[1] * Math.PI / 180
      const center = [command.start[0] - command.radius * Math.cos(startAngle), command.start[1] - command.radius * Math.sin(startAngle)]
      const end: [number, number] = [center[0] + command.radius * Math.cos(endAngle), center[1] + command.radius * Math.sin(endAngle)]
      const [x1, y1] = map(command.start)
      const [x2, y2] = map(end)
      const radius = command.radius * scale
      svg.append(svgNode('path', { d: `M ${x1} ${y1} A ${radius} ${radius} 0 ${Math.abs(command.angles[1] - command.angles[0]) > 180 ? 1 : 0} 0 ${x2} ${y2}`, fill: 'none', stroke: color, 'stroke-width': widthValue }))
      const middle = (startAngle + endAngle) / 2
      labelPoint = map([center[0] + command.radius * Math.cos(middle), center[1] + command.radius * Math.sin(middle)])
    } else if (command.type === 'rectangle') {
      const [x1, y1] = map(command.start)
      const [x2, y2] = map(command.end)
      svg.append(svgNode('rect', { x: Math.min(x1, x2), y: Math.min(y1, y2), width: Math.abs(x2 - x1), height: Math.abs(y2 - y1), rx: /rounded/.test(command.options) ? 12 : 0, fill: /fill=/.test(command.options) ? 'rgba(147, 197, 253, .16)' : 'none', stroke: color, 'stroke-width': widthValue }))
      labelPoint = [(x1 + x2) / 2, (y1 + y2) / 2]
    } else if (command.type === 'circle') {
      const [cx, cy] = map(command.start)
      svg.append(svgNode('circle', { cx, cy, r: command.radius * scale, fill: /fill=/.test(command.options) ? 'rgba(134, 239, 172, .16)' : 'none', stroke: color, 'stroke-width': widthValue }))
      labelPoint = [cx, cy]
    }
    if (command.label?.text) {
      const text = svgNode('text', { x: labelPoint[0] + (/right/.test(command.label.position) ? 9 : 0), y: labelPoint[1] - (/above/.test(command.label.position) ? 9 : 0), fill: color, 'font-size': 22 })
      text.textContent = command.label.text
      svg.append(text)
    }
  })
  ;[...resolvedContents.matchAll(/\\node(?:\[([^\]]*)\])?\s*at\s*\((-?\d*\.?\d+),\s*(-?\d*\.?\d+)\)\s*\{([^{}]*)\}/g)].forEach((match) => {
    const [x, y] = map([Number(match[2]), Number(match[3])])
    const text = svgNode('text', { x, y, fill: tikzColor(match[1] || ''), 'font-size': 21, 'text-anchor': 'middle' })
...
function safePlotFunction(expression) {
  const normalized = expression
    ...
    .trim()
  const operatorsOnly = normalized.replace(/Math\.(?:sin|cos|tan|exp|log)/g, '').replace(/x/g, '')
...
function makeFunctionPlot(contents) {
  const axis = contents.match(/\\begin\{axis\}(?:\[([^\]]*)\])?([\s\S]*?)\\end\{axis\}/)
  ...
  const domainMatch = plot[1]?.match(/domain\s*=\s*(-?\d*\.?\d+)\s*:\s*(-?\d*\.?\d+)/)
  ...
  const height = 300
  const padding = 42
  const mapX = (x) => padding + (x - from) / Math.max(.0001, to - from) * (width - padding * 2)
  const mapY = (y) => height - padding - (y - minY) / Math.max(.0001, maxY - minY) * (height - padding * 2)
  const svg = svgNode('svg', { viewBox: `0 0 ${width} ${height}`, role: 'img', 'aria-label': `График ${plot[2].trim()}` })
  for (let index = 0; index <= 8; index += 1) {
    const x = padding + index * (width - padding * 2) / 8
    const y = padding + index * (height - padding * 2) / 8
    svg.append(svgNode('line', { x1: x, y1: padding, x2: x, y2: height - padding, stroke: 'currentColor', 'stroke-opacity': .12, 'stroke-width': 1 }))
    svg.append(svgNode('line', { x1: padding, y1: y, x2: width - padding, y2: y, stroke: 'currentColor', 'stroke-opacity': .12, 'stroke-width': 1 }))
  }
  if (from <= 0 && to >= 0) svg.append(svgNode('line', { x1: mapX(0), y1: padding, x2: mapX(0), y2: height - padding, stroke: 'currentColor', 'stroke-width': 1.6 }))
  if (minY <= 0 && maxY >= 0) svg.append(svgNode('line', { x1: padding, y1: mapY(0), x2: width - padding, y2: mapY(0), stroke: 'currentColor', 'stroke-width': 1.6 }))
  const points = samples.map(([x, y]) => `${mapX(x)},${mapY(y)}`).join(' ')
  svg.append(svgNode('polyline', { points, fill: 'none', stroke: tikzColor(plot[1] || ''), 'stroke-width': /thick/.test(plot[1] || '') ? 2.7 : 2, 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }))
  wrapper.append(svg)
  return wrapper
}
...
function errorHtml(error, source = '') {
  ...
  const sourceLine = location ? source.split('\n')[location.line - 1]?.trim() : ''
...
export function renderTex(source, settings, fontPool) {
  let parsedSource = ''
  try {
    const { prepared, tables, multicols, tikzPictures, codeBlocks, boxes, circuits } = prepareSource(source)
    const generator = new HtmlGenerator({ hyphenate: false })
    ...
    replaceTokenBlock(container, RULE_TOKEN, () => document.createElement('hr'))
    multicols.forEach((multicol, index) => {
      replaceTokenBlock(container, `${MULTICOL_TOKEN_PREFIX}${index}`, () => makeMulticols(multicol))
    })
    tikzPictures.forEach((picture, index) => {
      replaceTokenBlock(container, `${TIKZ_TOKEN_PREFIX}${index}`, () => makeTikzPicture(picture))
    })
    circuits.forEach((_circuit, index) => {
      replaceTokenBlock(container, `${CIRCUIT_TOKEN_PREFIX}${index}`, () => makeCircuit())
    })
    boxes.forEach((box, index) => {
      replaceTokenBlock(container, `${BOX_TOKEN_PREFIX}${index}`, () => makeBox(box))
    })
    codeBlocks.forEach((block, index) => {
      replaceTokenBlock(container, `${CODE_TOKEN_PREFIX}${index}`, () => makeCodeBlock(block))
    })
    tables.forEach((table, index) => {
      replaceTokenBlock(container, `${TABLE_TOKEN_PREFIX}${index}`, () => makeTable(table))
    })
    const body = container.querySelector('.body') || container
    return renderHandwrittenHtml(body.innerHTML, settings, fontPool)
  } catch (error) {
    return renderHandwrittenHtml(errorHtml(error, parsedSource), settings, fontPool)
  }
}
